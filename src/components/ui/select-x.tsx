...
export interface SelectXHandle {
  selectedIds: string[]
}

export const SelectX = forwardRef<SelectXHandle, SelectXProps>(
  (
    {
      options,
      value,
    ...
    useImperativeHandle(ref, () => ({
      selectedIds: options.filter((opt) => value.includes(opt.value)).map((opt) => opt.id)
    }))
          ...
          >
            {value.length === 0 ? (
              <div className="w-full flex items-center justify-between">
                <span className="text-sm text-secondary dark:text-darksecondary">
                  {placeholder}
                </span>
                <IconSelf
